...
public class BaselineAnnotator extends FakeAnnotator {
	...
	public HashSet<ScoredAnnotation> solveSa2W(String text) throws AnnotationException {
        ...
        while (it.hasNext()) {
            ...
            if (usedMentions.stream().allMatch(
                m -> ((mentionStart < m.first || mentionStart > m.second) &&
                ...
                if (mentionProbability > 0) {
                    ...
                    scoredAnnotations.add(
                        new ScoredAnnotation(
                            candidate.getQueryStartPosition(),
                            mention.length(),
                            wikiId,
                            // We just use the highest commonness as the annotation score (score
                            // doesn't matter too much for us in the C2W and A2W benchmarks).
                            (float) highestScore)
                    );
                }
            }
